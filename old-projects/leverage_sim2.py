...
class LoanSimulator:
    ...
    def suggest_viable_parameters(self, start_price: float, goal_btc: float) -> dict:
        ...
        months_to_goal = (goal_btc * start_price) / monthly_dca if monthly_dca > 0 else float('inf')
        suggestions.append({
...
def main():
    ...
    investment_value = 30000.0  # Always start with $30k worth of BTC
    start_btc = investment_value / start_price  # Calculate BTC amount based on current price
    btc_goal = 1.0  # Target 1.0 BTC total

    print(f"Current BTC Price: ${start_price:,.2f}")
    print(f"Starting with ${investment_value:,.0f} worth of BTC: {start_btc:.6f} BTC")
    ...
    while total_btc < btc_goal and cycle < 30:  # Safety limit of 30 cycles
        ...
        # FIX: Implemented progressive loan sizing per the logic fix plan.
        # Collateral is now half of total BTC, and the loan is sized dynamically.
